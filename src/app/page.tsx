      ...
      {units.map(({ label, value, clickable }, i) => (
        <div key={label} className="flex items-center gap-1.5 sm:gap-3">
          <div
            ...
            className={`flex min-w-[52px] flex-col items-center rounded-2xl border border-[#0000CD]/15 bg-white/55 px-2 py-2 backdrop-blur-sm transition-colors sm:min-w-[72px] sm:px-3 sm:py-2.5 ${
              clickable ? 'cursor-pointer hover:bg-white/80 hover:border-[#0000CD]/30' : ''
            }`}
          >
            <span className="text-xl font-bold tabular-nums text-[#0000CD] sm:text-3xl">
              {value == null ? '--' : String(value).padStart(2, '0')}
            </span>
            <span className="mt-0.5 text-[9px] font-semibold uppercase tracking-widest text-[#0000CD]/50 sm:text-[10px]">
              {label}
            </span>
          </div>
          {i < 3 && (
            <span className="mb-2 text-base font-bold text-[#0000CD]/30 sm:mb-3 sm:text-xl">:</span>
          )}
        </div>
      ))}
    </div>
    ...
    <div className="min-h-screen bg-white">
      ...
      */}
      ...
      >
        ...
        {/* Hero content */}
        <div className="relative z-10 flex flex-1 flex-col items-center justify-center px-4 pb-20 pt-4 text-center sm:pb-24 sm:pt-8">
          {/* Date + location badge */}
          <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-[#0000CD]/20 bg-white/50 px-4 py-2 text-xs font-semibold text-[#0000CD] backdrop-blur-sm sm:mb-7 sm:gap-2.5 sm:px-5 sm:py-2.5 sm:text-sm">
          ...
          <h1 className="mb-4 max-w-4xl text-3xl font-bold italic leading-tight tracking-tight text-[#0000CD] sm:mb-6 sm:text-5xl md:text-7xl lg:text-8xl">
            Wandeling 2026
          </h1>
          ...
          {/* Scroll hint */}
          <a href="#programma" className="absolute bottom-20 left-1/2 flex -translate-x-1/2 cursor-pointer flex-col items-center gap-1.5 text-[#0000CD]/40 transition-colors hover:text-[#0000CD]/70 sm:bottom-24">
